...
export function pick() {
  ...
  // document.body.insertBefore(svg, prev1Div);
  document.addEventListener('mousemove', (event) => {
    ...
    hslDiv.textContent = `hsl(${Math.round(hueVal)}, 50%, ${Math.round(
      lumVal
    )}%)`;
    document.body.style.background = `hsl(${Math.round(
      hueVal
    )}, 50%, ${Math.round(lumVal)}%)`;
